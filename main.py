...
#get the pushed match total
def get_push():
  pushtot = 0
  if "pool_push" in db.keys():
    pushtot = pushtot + db["pool_push"]
  return pushtot
...
async def wrestler(self, name):
    ...
    if "wrestlername" in db.keys():
        ...
        if name in names:
            await self.send(
                "Wrestler {0} already exists in the database!".format(name))
        else:
...
async def wrestlerlist(self):
    ...
    troubleshootpush = "current pust total "
    if "wrestlername" in db.keys():
    ...
    print(troubleshootpush)
    message = "List of current wrestlers "
    if "wrestlername" in db.keys():
        for x in range(len(db["wrestlername"])):
            message = message + db["wrestlername"][x] + "(" + str(
                db["wrestlerwins"][x]) + "," + str(
                    db["wrestlerloss"][x]) + "), "
        await self.send(message)
    else:
...
async def delwrestler(self, name):
    ...
    if "wrestlername" in db.keys():
        ...
        if name in names:
            ...
            await self.send(
                'No wrestler with name {0} in database'.format(name))
    else:
        await self.send(empty)
...
async def setrecord(self, wrestler, wins, loss):
    print('setrecord command accepted with {0} wins and {1} losses'.format(
        wins, loss))
    if "wrestlername" in db.keys():
        names = db["wrestlername"]
        if wrestler in names:
            ...
            await self.send(
                "Wrestler {0} record adjusted to (".format(wrestler) +
                str(db["wrestlerwins"][index]) + "," +
                str(db["wrestlerloss"][index]) + ")")
        else:
            await self.send(
                "No wrestler with name {0} found. Use !list to see a list of currently registered wrestlers."
                .format(wrestler))
    else:
        await self.send(empty)
...
async def record(self, wrestler):
    ...
    if "wrestlername" in db.keys():
        ...
        if wrestler in names:
            index = get_index(wrestler)
            await self.send(db["wrestlername"][index] + "(" +
                            str(db["wrestlerwins"][index]) + "," +
                            str(db["wrestlerloss"][index]) + ")")
        else:
            await self.send(
                "No wrestler with name {0} found. Use !wrestlerlist to see a list of currently registered wrestlers."
                .format(wrestler))
    else:
        await self.send(empty)
...
async def match(self, *wrestlers):
    print('match command accepted with ' +
          '{} arguments: {}'.format(len(wrestlers), ', '.join(wrestlers)))
    if "wrestlername" in db.keys():
        names = db["wrestlername"]
        if set(wrestlers).issubset(set(names)):
            ...
            for x in wrestlers:
                ...
                message = message + x + " (" + str(
                    db["wrestlerwins"][index]) + "," + str(
                        db["wrestlerloss"][index]) + ") "
            ...
            await self.send(
                "One of the wrestlers you entered doesnt match the database; this is where it would tell you which one if i was a good programmer"
            )
    else:
        await self.send(empty)
...
async def win(self, winner):
    ...
    if "contenders" in db.keys():
        ...
        if winner in cont:
            ...
            message = winner + " wins! Record updated to (" + str(
                db["wrestlerwins"][index]) + "," + str(
                    db["wrestlerloss"][index]) + ") "
            ...
            if users:
                ...
                message = "No one bet on winner... Pool pushed to next match (please work...) "
                push_match()
            cont.remove(winner)
            for x in cont:
                if x in db["wrestlername"]:
                    ...
                    set_loss(x, l)
                    index = get_index(x)
                    message = message + x + " lost! Record updated to (" + str(
                        db["wrestlerwins"][index]) + "," + str(
                            db["wrestlerloss"][index]) + ") "
            await self.send(message)
            wipe()
...
async def setbal(self, user, x):
    print('setbank command accepted with name {0} and ammount {1}'.format(
        user, x))
    if "users" in db.keys():
        names = db["users"]
        if user in names:
            index = get_user_index(user)
            set_bank(user, x)
            await self.send("User {0}'s bank set to ".format(user) +
                            str(db["bank"][index]))
        else:
            await self.send(
                "User {0} doesn't exist in user database".format(user))
    else:
...
async def bal(self):
    ...
    if "users" in db.keys():
        ...
        if name in names:
            ...
            await self.send("{0}'s current balence: {1}".format(
                name, str(db["bank"][index])))
        else:
...
async def bet(self, contender):
    ...
    print("Bet command accepted from {0} with {1} variable".format(
        user, contender))
        ...
        for x in range(lendex):
            if x == 0:
                if "contender0" in db.keys():
                    ...
                    for y in bet0:
                        better0 = better0 + y + ", "
                    message = message + contenders[
                        x] + "'s current pool is " + str(
                            pool[x]) + " with" + better0 + "betting on them.\n"
            if x == 1:
                if "contender1" in db.keys():
                    bet1 = db["contender1"]
                    better1 = " "
                    ...
                    message = message + contenders[
                        x] + "'s current pool is " + str(
                            pool[x]) + " with" + better1 + "betting on them.\n"
            if x == 2:
                if "contender2" in db.keys():
                    bet2 = db["contender2"]
                    better2 = " "
                    ...
                    message = message + contenders[
                        x] + "'s current pool is " + str(
                            pool[x]) + " with" + better2 + "betting on them.\n"
            if x == 3:
                if "contender3" in db.keys():
                    bet3 = db["contender3"]
                    better3 = " "
                    ...
                    message = message + contenders[
                        x] + "'s current pool is " + str(
                            pool[x]) + " with" + better3 + "betting on them.\n"
            if x == 4:
                if "contender4" in db.keys():
                    bet4 = db["contender4"]
                    better4 = " "
                    ...
                    message = message + contenders[
                        x] + "'s current pool is " + str(
                            pool[x]) + " with" + better4 + "betting on them.\n"
            if x == 5:
                if "contender5" in db.keys():
                    bet5 = db["contender5"]
                    better5 = " "
                    ...
                    message = message + contenders[
                        x] + "'s current pool is " + str(
                            pool[x]) + " with" + better5 + "betting on them.\n"
            if x == 6:
                if "contender6" in db.keys():
                    bet6 = db["contender6"]
                    better6 = " "
                    ...
                    message = message + contenders[
                        x] + "'s current pool is " + str(
                            pool[x]) + " with" + better6 + "betting on them.\n"
            if x == 7:
                if "contender7" in db.keys():
                    bet7 = db["contender7"]
                    better7 = " "
                    ...
                    message = message + contenders[
                        x] + "'s current pool is " + str(
                            pool[x]) + " with" + better7 + "betting on them.\n"
        await self.send(message)
...
async def namechange(self, oldname, newname):
    if "wrestlername" in db.keys():
        ...
        if oldname in names:
            ...
            await self.send(
                "{0} doesn't exist in the database!".format(oldname))
...
async def push(self):
    ...
    wipe()
    await self.send(
        "Betting pool pushed to next match with a toal of {0}!".format(totpool)
    )
